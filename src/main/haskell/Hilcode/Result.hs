...
instance (Eq failure, Eq value) => Eq (Result failure value) where
    (==) :: Result failure value -> Result failure value -> Bool
    Ok lhs == Ok rhs = lhs == rhs
    Err lhs == Err rhs = lhs == rhs
    _ == _ = False

instance (Ord failure, Ord value) => Ord (Result failure value) where
    (<=) :: Result failure value -> Result failure value -> Bool
    Ok lhs <= Ok rhs = lhs <= rhs
    Err lhs <= Err rhs = lhs <= rhs
    Err _ <= Ok _ = True
