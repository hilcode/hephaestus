...
mkGlob2 :: Text -> Result GlobError Glob
mkGlob2 originalText =
    let
        text :: OsString
        text = System.OsPath.unsafeEncodeUtf (unpack originalText)
     ...
     in
        case Hilcode.Parser.toResult $ Hilcode.Parser.runParser parseGlob text of
            Ok (Just (rest, glob))
                | Data.List.null rest ->
                    Ok glob
            Err parseError ->
...
mkGlob :: Text -> Result GlobError Glob
mkGlob text =
    let
        ...
        osPath =
            Hilcode.Result.mapError
                ...
                (Hilcode.Result.fromEither $ System.OsPath.encodeWith System.IO.utf8 System.IO.utf16 (unpack text))
